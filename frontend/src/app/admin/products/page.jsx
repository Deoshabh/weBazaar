...
export default function AdminProductsPage() {
  ...
  const fetchProducts = async (page = 1, search = '') => {
    try {
      ...
      const response = await adminAPI.getAllProducts({ page, limit: 20, search });
      const data = response.data;
      // Backend now returns { products, total, page, totalPages }
  ...
  // Client-side status filtering on the current page
              ...
              <div className="flex gap-2 flex-wrap">
                {['all', 'active', 'inactive'].map((status) => (
                  <button
                    ...
                    className={`px-3 sm:px-4 py-2 rounded-lg font-medium text-sm transition-colors touch-manipulation ${filterStatus === status
                      ...
                      <tr key={product._id} className="hover:bg-primary-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <button
                            ...
                            className={`flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${product.status === 'active'
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-6 bg-white rounded-lg shadow-md p-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => { setCurrentPage(p => p - 1); fetchProducts(currentPage - 1, searchQuery); }}
                  disabled={currentPage <= 1}
                  className="px-4 py-2 text-sm border border-primary-200 rounded-lg hover:bg-primary-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  Previous
                </button>
                <button
